...
[FermiDiracOperatorExpansion](https://github.com/singularitti/FermiDiracOperatorExpansion.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("FermiDiracOperatorExpansion")
   ```

3. Run

   ```julia-repl
   julia> using FermiDiracOperatorExpansion
   ```
...
version of FermiDiracOperatorExpansion, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/FermiDiracOperatorExpansion.jl"
```
...
our [GitHub repository](https://github.com/singularitti/FermiDiracOperatorExpansion.jl)
for new releases.
Once we release a new version, you can update FermiDiracOperatorExpansion by typing

```@repl
using Pkg
Pkg.update("FermiDiracOperatorExpansion")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("FermiDiracOperatorExpansion")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall FermiDiracOperatorExpansion.
