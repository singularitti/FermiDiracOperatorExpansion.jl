...
!!! info
    The style guide is always a work in progress, and not all FermiDiracOperatorExpansion code
    follows the rules. When modifying FermiDiracOperatorExpansion, please fix the style violations
...
FermiDiracOperatorExpansion uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/FermiDiracOperatorExpansion.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the FermiDiracOperatorExpansion directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to FermiDiracOperatorExpansion have
